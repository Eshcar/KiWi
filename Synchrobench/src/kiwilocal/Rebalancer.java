...
public class Rebalancer<K extends Comparable<? super K>,V> {
    ...
    class PolicyImpl extends Policy{
        ...
        private static final double MAX_AFTER_MERGE_PART= 0.5;
    ...
    private AtomicInteger freezedItems = new AtomicInteger(Integer.MIN_VALUE);
   ...
   /******* Constructors *********/
    ...
    public Rebalancer<K,V> engageChunks()
    {
        ...
        p.updateRangeView();
        engagedChunks.compareAndSet(null,createEngagedList(p.getFirstChunkInRange())); // if CAS fails here - another thread has updated it
    ...
    public Rebalancer freeze()
    {
        ...
        int totalFreezedItems = 0;

        for(Chunk<K,V> chunk : getEngagedChunks()){
            totalFreezedItems += chunk.freeze();
        }

        freezedItems.set(totalFreezedItems);
    ...
    public Rebalancer compact(ScanIndex<K> scanIndex)
    {
        ...
        List<Chunk<K,V>> engaged = getEngagedChunks();
        List<Chunk<K,V>> compacted =  c.compact(getEngagedChunks(),scanIndex);

        int maxVersion = 0;
        for(Chunk<K,V> chunk : engaged)
        {
            int local = chunk.localVersion.get();
            maxVersion =  local > maxVersion ? local : maxVersion;
        }

        for(Chunk<K,V> chunk : compacted)
        {
            chunk.localVersion.set(maxVersion);
        }
